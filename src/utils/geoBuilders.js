  ...
  return makeTextTexture((ctx, w, h) => {
    ...
    for (let x = 0; x < cols; x++) {
      for (let y = 0; y < rows; y++) {
        ctx.fillStyle = Math.random() < lit ? litColor : dimColor;
...
export function createTree({ height = 4, radius = 1.4, trunkColor = 0x6b4a2f, leafColor = 0x2f9e4f } = {}) {
  ...
  const trunk = mesh(new THREE.CylinderGeometry(radius * 0.14, radius * 0.2, trunkH, 6), trunkColor, { roughness: 0.95 });
  ...
  for (let i = 0; i < tiers; i++) {
    ...
    const cone = mesh(new THREE.ConeGeometry(r, h, 7), leafColor, { roughness: 0.9 });
...
export function createPalmTree({ height = 6, trunkColor = 0x8a6a42, leafColor = 0x3c9a4a } = {}) {
  ...
  for (let i = 0; i < segs; i++) {
    ...
    const seg = mesh(new THREE.CylinderGeometry(0.14 - i * 0.01, 0.17 - i * 0.01, segH, 5), trunkColor, { roughness: 0.9 });
  ...
  for (let i = 0; i < frondCount; i++) {
    ...
    const frond = mesh(new THREE.ConeGeometry(0.32, 2.6, 4), leafColor, { roughness: 0.85 });
  ...
  const coco = mesh(new THREE.SphereGeometry(0.35, 6, 5), 0x5b3a20, { roughness: 0.9 });
...
export function createBush({ radius = 0.8, color = 0x3c9a4a } = {}) {
  const g = new THREE.Group();
  for (let i = 0; i < 3; i++) {
    const s = mesh(new THREE.IcosahedronGeometry(radius * (0.7 + Math.random() * 0.4), 0), color, { roughness: 0.9 });
    s.position.set((Math.random() - 0.5) * radius, radius * 0.4 + Math.random() * 0.2, (Math.random() - 0.5) * radius);
    g.add(s);
  }
  g.traverse((o) => { if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; } });
  return g;
...
export function createLampPost({ height = 4.2, color = 0x2b2d30, lampColor = 0xfff2b0, lit = true } = {}) {
  const g = new THREE.Group();
  const pole = mesh(new THREE.CylinderGeometry(0.07, 0.09, height, 6), color, { roughness: 0.6, metalness: 0.4 });
  pole.position.y = height / 2;
  g.add(pole);
  const arm = mesh(new THREE.BoxGeometry(0.5, 0.06, 0.06), color, { roughness: 0.6, metalness: 0.4 });
  arm.position.set(0.25, height - 0.1, 0);
  g.add(arm);
  const lampGeo = new THREE.SphereGeometry(0.16, 8, 8);
  const lamp = mesh(lampGeo, lampColor, { emissive: lampColor, emissiveIntensity: lit ? 1.4 : 0.1, roughness: 0.4 });
...
export function createCloud({ scale = 1, color = 0xffffff } = {}) {
  ...
  for (let i = 0; i < n; i++) {
    const s = mesh(new THREE.SphereGeometry(scale * (0.6 + Math.random() * 0.5), 7, 6), color, { roughness: 1, emissive: color, emissiveIntensity: 0.08 });
    s.position.set(i * scale * 1.1 - n * scale * 0.5, Math.random() * scale * 0.3, Math.random() * scale * 0.4);
...
export function createWindmill({ height = 9, bodyColor = 0xd9c9a3, capColor = 0x5b4636, bladeSpan = 6.5 } = {}) {
  const g = new THREE.Group();
  const body = mesh(new THREE.CylinderGeometry(1.6, 2.4, height, 8), bodyColor, { roughness: 0.9 });
  body.position.y = height / 2;
  g.add(body);
  const cap = mesh(new THREE.ConeGeometry(1.9, 2.2, 8), capColor, { roughness: 0.8 });
  cap.position.y = height + 1.0;
  g.add(cap);
  for (let i = 0; i < 3; i++) {
    const r = 1.6 - i * (1.6 - 2.4) / 20;
    const ring = mesh(new THREE.CylinderGeometry(r, r, 0.4, 8, 1, true), 0xc9b98d, { roughness: 0.95, side: THREE.DoubleSide });
  ...
  const hubCap = mesh(new THREE.SphereGeometry(0.35, 8, 8), 0x3a2c1e, { roughness: 0.7 });
  hub.add(hubCap);
  const blades = new THREE.Group();
  for (let i = 0; i < 4; i++) {
    const blade = new THREE.Group();
    const beam = mesh(new THREE.BoxGeometry(0.18, bladeSpan, 0.06), 0x4a3826, { roughness: 0.9 });
    beam.position.y = bladeSpan / 2;
    blade.add(beam);
    for (let s = 1; s < 5; s++) {
      const lat = mesh(new THREE.BoxGeometry(0.9, 0.05, 0.04), 0x4a3826, { roughness: 0.9 });
      lat.position.y = (bladeSpan / 5) * s;
      blade.add(lat);
    }
    const sail = mesh(new THREE.PlaneGeometry(0.85, bladeSpan * 0.82), 0xece0c4, { roughness: 1, side: THREE.DoubleSide, castShadow: false });
...
export function createCanalHouse({ width = 3.4, depth = 3.6, floors = 3, floorH = 1.5, color = 0xb5502e, trimColor = 0xe9dfc4, doorColor = 0x2b3a2e, awningColor = 0x8c3a24, steps = 3 } = {}) {
  ...
  const body = mesh(new THREE.BoxGeometry(width, wallH, depth), color, { roughness: 0.95 });
  body.position.y = wallH / 2;
  g.add(body);

  // Trim strip between floors
  for (let f = 1; f < floors; f++) {
    const trim = mesh(new THREE.BoxGeometry(width + 0.05, 0.08, depth + 0.05), trimColor, { roughness: 0.9 });
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const block = mesh(new THREE.BoxGeometry(stepW * 1.02, stepH, 0.4), trimColor, { roughness: 0.9 });
    block.position.set(0, wallH + i * stepH + stepH / 2, depth / 2 + 0.15);
    g.add(block);
    const blockL = mesh(new THREE.BoxGeometry(0.3, stepH, 0.42), trimColor, { roughness: 0.9 });
  ...
  const cap = mesh(new THREE.ConeGeometry(0.36, 0.7, 4), trimColor, { roughness: 0.9 });
  ...
  for (const side of [-1, 1]) {
    const slab = mesh(new THREE.BoxGeometry(slopeLen, 0.16, depth * 0.98), C_ROOF, { roughness: 0.9 });
  ...
  // Windows per floor
  const winMat = stdMat(0x1c2b33, { roughness: 0.3, metalness: 0.2, emissive: 0xfff2c9, emissiveIntensity: 0.15 });
  const frameMat = stdMat(0xffffff, { roughness: 0.8 });
  for (let f = 0; f < floors; f++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      const win = new THREE.Mesh(winGeo, isDoor ? stdMat(doorColor, { roughness: 0.7 }) : winMat);
      ...
      g.add(win);
      const frame = mesh(new THREE.BoxGeometry(ww + 0.08, wh + 0.08, 0.05), trimColor, { roughness: 0.8 });
      ...
      if (!isDoor) {
        for (const side of [-1, 1]) {
          const shutter = mesh(new THREE.BoxGeometry(ww * 0.22, wh, 0.04), 0x2b4a3a, { roughness: 0.9 });
    ...
    if (f === 0) {
      const awning = mesh(new THREE.CylinderGeometry(0.34, 0.34, width * 0.78, 6, 1, false, 0, Math.PI), awningColor, { roughness: 0.85 });
...
export function createBuildingBlock({ w = 6, h = 20, d = 6, color = 0xd8d8d8, lit = true } = {}) {
  ...
  g.add(body);
  const roof = mesh(new THREE.BoxGeometry(w * 1.02, 0.3, d * 1.02), 0x3a3a3f, { roughness: 0.8 });
...
export function createSupertree({ height = 14, canopyRadius = 3.2, trunkColor = 0x3a2e22, canopyColor = 0x2fae6a } = {}) {
  const g = new THREE.Group();
  const trunk = mesh(new THREE.CylinderGeometry(0.5, 0.9, height, 8), trunkColor, { roughness: 0.85 });
  ...
  for (let i = 0; i < tiers; i++) {
    ...
    const ring = mesh(new THREE.CylinderGeometry(r, r * 0.85, 0.5, 10), canopyColor, { roughness: 0.7, emissive: canopyColor, emissiveIntensity: 0.25 });
...
export function createTulip({ color = 0xe5395a } = {}) {
  const g = new THREE.Group();
  const stem = mesh(new THREE.CylinderGeometry(0.02, 0.025, 0.35, 4), 0x3f8f3f, { roughness: 0.9, castShadow: false });
  stem.position.y = 0.175;
  g.add(stem);
  const bulb = mesh(new THREE.SphereGeometry(0.09, 6, 6), color, { roughness: 0.6, castShadow: false });
...
export function createBike({ color = 0x2b2d30 } = {}) {
  ...
  const w1 = mesh(wheelGeo, 0x111214, { roughness: 0.6, castShadow: false });
  w1.rotation.y = Math.PI / 2;
  w1.position.set(-0.5, 0.32, 0);
  const w2 = w1.clone(); w2.position.x = 0.5;
  g.add(w1, w2);
  const frame = mesh(new THREE.BoxGeometry(1.05, 0.04, 0.04), color, { roughness: 0.5, metalness: 0.5, castShadow: false });
  frame.position.set(0, 0.5, 0);
  frame.rotation.z = 0.15;
  g.add(frame);
  const seatPost = mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.35, 5), color, { metalness: 0.5, castShadow: false });
  seatPost.position.set(-0.35, 0.68, 0);
  g.add(seatPost);
  const handlePost = mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.4, 5), color, { metalness: 0.5, castShadow: false });
